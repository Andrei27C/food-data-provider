...
int main() {
    ...
    //for(int i=0;i<noOfFoods;i++){
        //printf("Food %s\n",foods[i]);
    //}
    int * noOfSpecificFood = (int*)malloc(noOfFoods * sizeof(int));
    char *** specificFood = (char***)malloc(noOfFoods * sizeof(char**));
    ...
    for(int i=0;i<noOfFoods;i++)
    {
        ...
        for(int j=0; j<noOfSpecificFood[i]; j++)
        {
            ...
            for(int k=0; k<MAX_LINE; k++)
                line[k] = '\0';
            //memset(line, '\0', sizeof(char) * MAX_LINE);
            gets(line);
            readLine(line, specificFood[i][j], &prices[i][j]);
        }
    }
    ...
    for(int i=0;i<noOfFoods;i++) {
        printf("%s: ", foods[i]);
        for(int j=0;j<noOfSpecificFood[i];j++) {
            printf("(%s- %.2lf) ", specificFood[i][j], prices[i][j]);
        }
        printf("\n");
    }
